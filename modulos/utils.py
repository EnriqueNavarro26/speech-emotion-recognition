...
def add_noise(data):
    """This function add noise to the audio."""
    noise_value = 0.015 * np.random.uniform() * np.amax(data)
    data = data + noise_value * np.random.normal(size=data.shape[0])
    return data

def shift_process(data):
    """This function shift the audio."""
    shift_range = int(np.random.uniform(low=-15,high=15) * 1000)
    return np.roll(data,shift_range)

def pitch_process(data,sampling_rate,pitch_factor=0.7):
    """This function change the pitch of the audio."""
    return librosa.effects.pitch_shift(data,sampling_rate,pitch_factor)

def change_speed(data, rate = 0.8):
    """This function change the speed of the audio."""
    return librosa.effects.time_stretch(data, rate = rate)

def extract_process(data, sample_rate):
    """This function extract the features of the audios.
        Parameters:
            - data: audio data. 
            - sample_rate: sample rate of the audio.
        Returns:
            - output_result: features of the audio file.
    """
...
def export_process(path, data_augmentation = False):
    """This function export the features of the audios. 
        Parameters: 
            - path: path of the audio file.
            - data_augmentation: False (default). If you want data augmentation, True.
        Returns:
            - result: features of the audio file.
    """

    if data_augmentation == False:
        data,sr = librosa.load(path)
        output_1 = extract_process(data,sr) # Extracting features without data augmentation
    ...
    result = np.vstack((result,output_3)) # Stacking vertically
    
    #Noise, shift_process
    noise_out = add_noise(data)
    shift_out = shift_process(noise_out)
    output_4 = extract_process(shift_out,sr)
    result = np.vstack((result,output_4)) # Stacking vertically
    
    # pitch_out= pitch_process(data, sr)
    # output_5 = extract_process(pitch_out,sr)
    # result = np.vstack((result,output_5))
    
    #Change_speed, pitch_process
    plus_out = change_speed(data)
    strectch_pitch = pitch_process(plus_out,sr)
    output_6 = extract_process(strectch_pitch,sr)
    result = np.vstack((result,output_6)) # Stack vertically
    
    return result



def data_preparation(data: pd.core.frame.DataFrame, Y_col: int=-1, standard_scaler: bool=True, one_hot_encoding: bool=False,
                    train_test: bool=True, split_rate: float=0.8, cv: bool=False, n_fold: int=5, shuffle: bool=True):
